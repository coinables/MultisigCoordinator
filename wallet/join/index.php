...
<div class="card border-primary mb-3" style="max-width: 100rem;" id="receiveblock" style="text-align: center;">
  ...
  <div class="card-body" style="text-align: center;">
	...
	<br>
	<h3>You've been invited!</h3> <h4>To join this wallet grab a spot below and then click Join.</h4>
	<br>
	...
	<label style="font-size: 18px;">Signatures Required to Spend: </label><input type="number" class="maininput" value="<?php echo $mofn;?>" style="width: 48px; font-size: 20px;" disabled>
	<br><br>
...
<?php
	...
	for($i=0;$i<$numsigners;$i++){
		...
		$disableswitch = $thistaken !== 0 ? 'disabled' : '';
		...
		echo '<div><label>Signer #'.$plusone.': </label><input type="text" class="maininput" id="sigalias'.$i.'" placeholder="Name/Alias (optional)" value="'.$thisaliasout.'" '.$disableswitch.'>  <button onclick="makeSigner('.$scriptswitch.','.$plusone.')" class="maininput btn btn-outline-warning" '.$disableswitch.'>'.$buttonswitch.'</button> &nbsp;</div>';
		 
	}
		
?>
    </div>
	<br><div id="joindiv"> <button class="maininput btn btn-outline-success" onclick='return joinwallet(this);'>Join Wallet</button></div>
...
<br><br><b>Share this wallet link with friends to let them join.<br>After all spaces have been filled this link can also be used to view and access the wallet. <Br> Anyone with the link can join(assuming spaces are still available).</b>
  ...
  <div style="width: 60%; margin: auto; font-size: 18px; text-align: left;">
	...
	<p>If you want to be one of the signers on this multisig wallet click on one(or more) of the "Make Me Signer#_" buttons.</p>
	<p>Aliases are random, feel free to change, leave as-is or omitted entirely.</p>
	<p>Once all spaces have been filled this wallet URL will become active and all signers can start generating addresses.</p>
	</div>
</div>
...
function joinwallet(elem){
	var numadded = document.getElementsByClassName("pubkeyin").length;
	if(numadded < 1){
		alert('You must pick a signer position before you can join.');
	} else {
		document.getElementById("joindiv").innerHTML = '<h6>You have successfully joined! Wallet will be enabled after all spaces have been filled.</h6>';
		...
		var walletid = "<?php echo $walletlinkexists ?>";
		pps = []; 
		aas = [];
		pki = [];
		aas[0] = document.getElementById("sigalias0").value;
		...
		$.ajax({
				 type: "POST",
				 url: 'join.php',
				 data: {wid: walletid, uid: getuid, p1: pps[1], p2: pps[2], p3: pps[3], p4: pps[4], p5: pps[5], p6: pps[6], p7: pps[7], p8: pps[8], p9: pps[9], p10: pps[10], a1: aas[0], a2: aas[1], a3: aas[2], a4: aas[3], a5: aas[4], a6: aas[5], a7: aas[6], a8: aas[7], a9: aas[8], a10: aas[9], pk1: pki[1], pk2: pki[2], pk3: pki[3], pk4: pki[4], pk5: pki[5], pk6: pki[6], pk7: pki[7], pk8: pki[8], pk9: pki[9], pk10: pki[10]},
				 success: function(data){
...
var walletidentifier = "<?php echo $walletlinkexists ?>";

function beginWatch(){
	var watchStatus = setInterval(checkStatus, 5000);
	var statusCount = 0;
